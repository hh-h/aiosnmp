...
async def test_snmp_timeout_error(host: str) -> None:
    async with Snmp(host=host, port=1) as snmp:
...
async def test_snmp_no_access(host: str, port: int) -> None:
    async with Snmp(host=host, port=port, community="public") as snmp:
...
async def test_snmp_not_writable(host: str, port: int) -> None:
    async with Snmp(host=host, port=port, community="private") as snmp:
